...
export default function Hunting({ session }) {
  ...
  const [logForm, setLogForm] = useState({ hunter_name:'', hunt_date: format(new Date(),'yyyy-MM-dd'), session:'am', animal_type:'Whitetail', sightings:0, harvest:'No harvest', notes:'' })

  useEffect(() => { loadAll() }, [])

  async function loadAll() {
    const [{ data: b }, { data: f }, { data: c }, { data: l }] = await Promise.all([
      ...
      supabase.from('hunt_logs').select('*, blind:hunting_blinds(name)').order('hunt_date', {ascending:false}).limit(10)
    ])
    if (b) setBlinds(b)
    if (f) setFeeders(f)
    if (c) setCameras(c)
  ...
  async function saveLog() {
    await supabase.from('hunt_logs').insert({ ...logForm, hunter_id: session.user.id, blind_id: selected?.id })
    loadAll()
    setTab('log')
  }
            ...
            {cameras.map(c => (
              <div key={c.id} className="card" style={{marginBottom:7}}>
                <div style={{display:'flex',alignItems:'center',gap:9}}>
                  <div style={{width:32,height:32,borderRadius:7,background:'#E6F1FB',color:'#185FA5',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>📷</div>
                  <div style={{flex:1}}>
