...
function MediaUploader({ entityType, entityId, session, onUploaded }) {
  ...
  return (
    <div>
      <input ref={fileRef} type="file" accept="image/*,video/*" multiple capture="environment"
  ...
  return (
    <div style={{marginBottom:12}}>
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{
          ...
          <div style={{color:'#fff',fontSize:12,textAlign:'center'}}>
            {lightbox.file_name} · {lightbox.uploaded_profile?.full_name}
          </div>
          <button onClick={e=>{e.stopPropagation();deleteMedia(lightbox.id,lightbox.storage_path);setLightbox(null)}}
            style={{padding:'8px 16px',borderRadius:8,border:'none',background:'#A32D2D',color:'#fff',fontSize:12,cursor:'pointer'}}>
            Delete
          </button>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>
        {media.map(m => (
          <div key={m.id} onClick={() => setLightbox(m)} style={{
            aspectRatio:'1',borderRadius:8,overflow:'hidden',
            background:'#f0f0f0',cursor:'pointer',position:'relative'
          }}>
            {m.file_type === 'video' ? (
              <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'#1a1a1a',flexDirection:'column',gap:4}}>
                <span style={{fontSize:24}}>▶</span>
                <span style={{fontSize:9,color:'#aaa'}}>{m.file_name?.split('.').pop()?.toUpperCase()}</span>
              </div>
            ) : m.url ? (
...
function CommentSection({ entityType, entityId, session, tableName, fkField }) {
  ...
  async function loadComments() {
    const { data } = await supabase
      .from(tableName)
      .select(`*, created_profile:profiles!${tableName}_created_by_fkey(full_name, avatar_initials)`)
      .eq(fkField, entityId)
  ...
  return (
    <div>
        ...
        {comments.length === 0 ? (
          ...
          <div key={c.id} style={{padding:'9px 0',borderBottom:'0.5px solid #f5f5f5'}}>
            ...
            <div style={{paddingLeft:29,marginTop:6}}>
              <MediaGallery entityType="comment" entityId={c.id} session={session}/>
            </div>
          </div>
        ))}
      </div>
      <div style={{display:'flex',gap:8,marginBottom:4}}>
        <input className="form-input" style={{flex:1}} value={text}
      ...
      <div style={{marginBottom:12}}>
        <MediaUploader entityType="comment" entityId={`${entityType}-${entityId}-comment-new`} session={session} onUploaded={loadComments}/>
      </div>
    </div>
  )
}
...
export default function Tracker({ session }) {
  ...
  async function loadAll() {
    const [{ data: prof }, { data: wo }, { data: p }, { data: n }] = await Promise.all([
      ...
      supabase.from('work_orders').select('*, created_profile:profiles!work_orders_created_by_fkey(full_name)').order('created_at', { ascending: false }),
  ...
  async function saveWorkOrder() {
    ...
    await supabase.from('audit_log').insert({
      ...
      action: isEditing ? 'work_order_updated' : 'work_order_created',
      entity_type: 'work_order', diff_json: { title: woForm.title }
    })
    resetWOForm()
    setView('list')
    loadAll()
  ...
  function openEditWO(wo) {
    setWoForm({
      ...
      category: wo.category || CATEGORIES[0], asset: wo.asset || '— select —',
      priority: wo.priority || 'med', assigned_to: wo.assigned_to_name || 'Unassigned',
      due_date: wo.due_date || '', photo_required: wo.photo_required || false,
      approval_required: wo.approval_required !== false
    })
    setIsEditing(true)
    setSelectedWO(wo)
    setView('new-wo')
  ...
  const isOwnerAdmin = canEdit(profile)
  ...
  if (view === 'new-wo') return (
    <div>
      ...
      <div className="content">
          ...
          <div style={{display:'flex',gap:5}}>
            {Object.entries(PRI_CONFIG).map(([k,v]) => (
              <button key={k} onClick={() => setWoForm({...woForm,priority:k})} style={{
                ...
                border:`${woForm.priority===k?'1.5px':'0.5px'} solid ${woForm.priority===k?v.color:'#ddd'}`,
        ...
        <button className="btn btn-primary" onClick={saveWorkOrder} disabled={!woForm.title.trim()}>
  ...
  if (view === 'wo-detail' && selectedWO) {
    ...
    const isCreator = selectedWO.created_by === session.user.id
    return (
      <div>
        <div className="topbar">
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            ...
            {(isOwnerAdmin || isCreator) && (
              <button onClick={() => openEditWO(selectedWO)} style={{background:'rgba(255,255,255,0.2)',border:'none',color:'#fff',padding:'6px 12px',borderRadius:6,fontSize:12,cursor:'pointer'}}>Edit</button>
            )}
          </div>
        </div>
        <div className="content">
          ...
          <div style={{marginBottom:12}}>
            <MediaUploader entityType="work_order" entityId={selectedWO.id} session={session} onUploaded={() => {}}/>
          </div>
          ...
          {/* Comments */}
          <CommentSection
            entityType="work_order" entityId={selectedWO.id} session={session}
            tableName="work_order_comments" fkField="work_order_id"
          />
  ...
  // ── PUNCH LIST DETAIL ──────────────────────────────────────
  if (view === 'punch-detail' && selectedPunch) {
    const isCreator = selectedPunch.created_by === session.user.id
    return (
      <div>
        <div className="topbar">
        ...
        <div className="content">
          <div style={{display:'flex',gap:8,marginBottom:12}}>
            <button onClick={() => { togglePunch(selectedPunch.id, selectedPunch.done); setSelectedPunch({...selectedPunch,done:!selectedPunch.done}) }} style={{
              flex:1,padding:10,borderRadius:8,
              border:`1.5px solid ${selectedPunch.done?'#3B6D11':'#ccc'}`,
              background:selectedPunch.done?'#EAF3DE':'none',
              color:selectedPunch.done?'#3B6D11':'#666',
              fontSize:12,cursor:'pointer',fontFamily:'inherit',fontWeight:500
            }}>{selectedPunch.done ? '✓ Completed — tap to undo' : 'Mark complete'}</button>
            {(isOwnerAdmin || isCreator) && (
              <button onClick={() => { deletePunchItem(selectedPunch.id); setSelectedPunch(null); setView('list'); setTab('punch') }} style={{
                padding:'10px 14px',borderRadius:8,border:'0.5px solid #ddd',
                background:'#FCEBEB',color:'#A32D2D',fontSize:12,cursor:'pointer',fontFamily:'inherit'
              }}>Delete</button>
            )}
          </div>
          ...
          <div style={{marginBottom:12}}>
            <MediaUploader entityType="punch_list" entityId={selectedPunch.id} session={session} onUploaded={() => {}}/>
          </div>

          {/* Comments */}
          <CommentSection
            entityType="punch_list" entityId={selectedPunch.id} session={session}
            tableName="punch_list_comments" fkField="punch_list_id"
          />
  ...
  return (
    <div>
      ...
      <div className="content">
          ...
          <>
            ...
            : filteredWOs.length === 0 ? (
              <div style={{textAlign:'center',padding:'32px 0'}}>
                ...
                <p style={{fontSize:14,fontWeight:600,marginBottom:4}}>No work orders</p>
            ...
            <div style={{background:'#fff',border:'0.5px solid #ddd',borderRadius:8,padding:'0 12px',marginBottom:10}}>
              ...
              : punch.map(p => (
                <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:'0.5px solid #f5f5f5'}}>
                  ...
                  <div style={{flex:1,fontSize:12,color:'#333',textDecoration:p.done?'line-through':'none',opacity:p.done?0.5:1,cursor:'pointer'}}
                    onClick={() => { setSelectedPunch(p); setView('punch-detail') }}>
                    {p.text}
                  </div>
        ...
        {tab === 'notes' && (
          <>
            ...
            <div style={{background:'#f8f8f8',borderRadius:8,padding:12,marginBottom:12}}>
              ...
              <button className="btn btn-primary" style={{marginBottom:4}} onClick={addNote} disabled={!newNote.text.trim()}>Save observation</button>
            </div>
              ...
              return (
                <div key={n.id} style={{background:'#fff',border:'0.5px solid #ddd',borderRadius:8,padding:11,marginBottom:8}}>
                  ...
                  <div style={{marginBottom:8}}>
                    <MediaUploader entityType="note" entityId={n.id} session={session} onUploaded={() => {}}/>
                  </div>
        ...
        {/* ── PHOTOS ── */}
        {tab === 'photos' && (
          <AllMediaGallery session={session}/>
        )}
      </div>
    </div>
  )
    ...
    <>
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{
          ...
          <div style={{color:'#fff',fontSize:12,textAlign:'center'}}>
            {lightbox.file_name} · {lightbox.uploaded_profile?.full_name} · {format(new Date(lightbox.created_at),'MMM d')}
          </div>
          <div style={{color:'rgba(255,255,255,0.5)',fontSize:11}}>Tap anywhere to close</div>
        </div>
      )}
      ...
      : filtered.length === 0 ? (
        <div style={{textAlign:'center',padding:'48px 0'}}>
          ...
          <p style={{fontSize:14,fontWeight:600,color:'#333',marginBottom:4}}>No media yet</p>
          <p style={{fontSize:12,color:'#888'}}>Photos and videos from work orders and notes will appear here.</p>
        </div>
      ) : (
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>
          {filtered.map(m => (
            <div key={m.id} onClick={() => setLightbox(m)} style={{
              ...
              {m.file_type === 'video' ? (
                <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'#1a1a1a',flexDirection:'column',gap:4}}>
                  <span style={{fontSize:24}}>▶</span>
                  <span style={{fontSize:9,color:'#aaa'}}>{m.entity_type}</span>
                </div>
              ) : m.url ? (
                <img src={m.url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
              ) : (
